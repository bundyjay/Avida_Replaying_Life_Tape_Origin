...
from data_dict_phase1_ttest import create_data_dict
from sort_csv_phase1_ttest import sort_csv 
...
def create_csv(
    ...
    with open(file_location, 'w+', newline='') as csvfile:
        ...
        csv_writer.writerow([
            ...
            'ancestor_seed', 
            'trait', 
            'shallow_evolved_population_value_log10',
            'deep_evolved_population_value_log10'
            ])
        for key, attributes in data_dict.items():
            values = data_dict[key].values()
            csv_writer.writerow([key, *values])

    log_name= 'create_csv_phase1_ttest_log.txt'
    text = (
...
def create_and_sort_csv():
    ...
    yield print(create_csv(
        data_dict= create_data_dict(
            log_name= 
                'data_dict_phase1_ttest_log.txt'
            ), 
        csv_name= 'phase1_ttest_raw.csv'
        ))
